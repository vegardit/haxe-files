...
class File {
   ...
   private function new(path:Path) {
      ...
      public function touch():Void {
         ...
         #if python
            python.Syntax.code("import pathlib; pathlib.Path({0}).touch(exist_ok=True)", path.toString());
         #elseif phantomjs
            js.phantomjs.FileSystem.touch(path.toString());
         #elseif php
            php.Syntax.code("touch({0})", path.toString());
         #elseif (sys || macro || nodejs)
            if (path.exists()) {
               #if cs
                  cs.system.io.File.SetLastWriteTime(path.toString(), cs.system.DateTime.Now);
               #elseif java
                  (new java.io.File(path.toString())).setLastModified(java.lang.System.currentTimeMillis());
               #elseif lua
                  final now = Date.now().getTime() / 1000.0;
                  lua.lib.luv.fs.FileSystem.utime(path.toString(), cast now, cast now);
               #elseif (nodejs && !macro)
                  final nowDate = Date.now();
                  js.node.Fs.utimesSync(path.toString(), nowDate, nowDate);
               #else
                  // Fallback: rewrite without changing content to update mtime
                  final p = path.toString();
                  final stat = sys.FileSystem.stat(p);
                  if (stat.size > 0) {
                     var ex:Null<ConcurrentException> = null;

                     var firstByte:Int;
                     var fin:Null<sys.io.FileInput> = null;
                     var fout:Null<sys.io.FileOutput> = null;
                     try {
                        fin = sys.io.File.read(p, true);
                        firstByte = fin.readByte();

                        fout = sys.io.File.update(p, true);
                        fout.writeByte(firstByte);
                        fout.flush();
                     } catch (e:Dynamic) {
                        ex = ConcurrentException.capture(e);
                     }
